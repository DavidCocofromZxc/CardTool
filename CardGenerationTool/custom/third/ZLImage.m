...
#pragma mark - 组合图片
+ (NSImage *)completeImages:(NSArray<CGTImage *> *)imgArray;{
    NSImage *togetherImg = [[NSImage alloc]initWithSize:CARD_SIZE];
    [togetherImg lockFocus];
    
    if(imgArray.count <= 0){
        return nil; //报错
    }
    CGTImage *image = imgArray.firstObject;
    //如果超过范围-//需要裁切
    if(![self comparisonSizeA:CARD_SIZE andSizeB:image.img.size]){
        
    }
    //不需要裁切
    CGContextRef imgContext = (CGContextRef)[[NSGraphicsContext currentContext] graphicsPort];
    CGFloat imgX = 0;
    for (CGTImage *img in imgArray) {
        CGImageRef imageRef = [self getCGImageRefFromNSImage:img.img];
        CGContextDrawImage(imgContext, NSMakeRect(img.x,
                                                  img.y,
                                                  img.img.size.width,
                                                  img.img.size.height), imageRef);
    }
    [togetherImg unlockFocus];
    return togetherImg;
}
...
+ (BOOL)comparisonSizeA:(CGSize )sizeA andSizeB:(CGSize )sizeB;{
...
//得到小图在大图中居中时需要的frame.origin
+ (CGPoint )getCenteredDisplayPointInSize:(CGSize )bigSize withSize:(CGSize )smallSize;{
    return CGPointMake(bigSize.width/2 - smallSize.width/2,
                       bigSize.height/2 - smallSize.height/2);
}
